...
function normalize(data) {
   ...
   for (let i = 0; i < data.length; i++) {
      input = scaleDown(scaleUp(data[i]));
      scaled.push(input);

      if (lowest > input.low) lowest = input.low;
      if (highest < input.high) highest = input.high;
   }

   for (let i = 0; i < scaled.length; i++) {
      anw.push({
         open: (scaled[i].open - lowest) / (highest / lowest),
         high: (scaled[i].high - lowest) / (highest / lowest),
         low: (scaled[i].low - lowest) / (highest / lowest),
         close: (scaled[i].close - lowest) / (highest / lowest)
      })
   }
